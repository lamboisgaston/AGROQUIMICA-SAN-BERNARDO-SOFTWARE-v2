...
function cultivoRealDesdeNombre(nombre = '') {
...
async function main() {
  const where = { cultivo: CONFIG.cultivo, semilleroLaboratorio: CONFIG.semilleroLaboratorio };
  ...
  for (const [nombre, presentacionEnvase, precioListaUsd] of PRODUCTOS) {
    const cultivoReal = cultivoRealDesdeNombre(nombre);
    const metadatosComerciales = `origen=${CONFIG.origen}; precio_lista_usd=${precioListaUsd}; utilidad_porcentaje=${CONFIG.utilidadPorcentaje}; categoria_interna=${CONFIG.cultivo}`;
    const descripcion = `${cultivoReal} · ${CONFIG.semilleroLaboratorio} · ${nombre} · ${presentacionEnvase} · ${metadatosComerciales}`;

...
function senasaSelectProducto(name, label, value = '') {
  const productosMip = senasaProductos.filter(productoEsSenasaMip);
  return `<label>${label}<select name="${name}" data-senasa-producto-select><option value="">Seleccione producto SENASA / MIP</option>${productosMip.map((p) => `<option value="${p.id}" ${String(value) === String(p.id) ? 'selected' : ''}>${escapeHtmlClient(p.nombre || '-')} · SENASA / ANMAT</option>`).join('')}</select></label>`;
}
function senasaSelectResolucion(name, label, value = '') {
  return `<label>${label}<select name="${name}" data-senasa-resolucion-select><option value="">Prefijado / papeles</option>${senasaResoluciones.map((r) => `<option value="${r.id}" ${String(value) === String(r.id) ? 'selected' : ''}>${escapeHtmlClient(r.productoNombre || '-')} · ${escapeHtmlClient(r.resolucionNumero || 's/res.')}</option>`).join('')}</select></label>`;
}
...
function senasaProductoTecnico(productoId) {
  ...
  if (!p) return {};
  return { productoNombre: p.nombre || '', principioActivo: p.principioActivo || '', concentracion: p.concentracion || '', habilitacionHabitual: p.habilitacionHabitual || '', resolucionSenasa: p.resolucionSenasa || '', fechaResolucionSenasa: senasaFecha(p.fechaResolucionSenasa), tipoSenasa: p.tipoSenasa || '', usoSenasa: p.usoSenasa || '' };
}
function senasaResolucionTecnica(resolucionId) {
  const r = senasaResoluciones.find((x) => String(x.id) === String(resolucionId));
  if (!r) return {};
  return { productoNombre: r.productoNombre || '', principioActivo: r.principioActivo || '', resolucionSenasa: r.resolucionNumero || '', fechaResolucionSenasa: senasaFecha(r.fechaResolucion) };
}
function renderSenasaListas() {
  ...
  $('#senasa-resoluciones').innerHTML = senasaResoluciones.length ? senasaResoluciones.map((r) => `<div class="item"><b>${escapeHtmlClient(r.productoNombre || '-')}</b><br>Principio: ${escapeHtmlClient(r.principioActivo || '-')} · Res. ${escapeHtmlClient(r.resolucionNumero || '-')} · ${senasaFecha(r.fechaResolucion) || '-'}<br><button data-senasa-res-eliminar="${r.id}">Eliminar</button></div>`).join('') : '<div class="item">Sin resoluciones prefijadas.</div>';
}
function renderSenasaResolucionForm() {
  const sel = $('#senasa-res-producto');
  if (!sel) return;
  const productosMip = senasaProductos.filter(productoEsSenasaMip);
  sel.innerHTML = '<option value="">Producto SENASA / MIP</option>' + productosMip.map((p) => `<option value="${p.id}">${escapeHtmlClient(p.nombre || '-')} · SENASA / ANMAT</option>`).join('');
}
function renderSenasaForm() {
  ...
  const avisoHtml = `
    <h4>Roedores</h4><div class="senasa-grid-tres">
      ${senasaInput('roedores.periodoDesde', 'Periodo desde', 'date', d.roedores?.periodoDesde || d.periodoDesde)}${senasaInput('roedores.periodoHasta', 'Periodo hasta', 'date', d.roedores?.periodoHasta || d.periodoHasta)}${senasaSelectProducto('roedores.productoId', 'Producto cebo rodenticida', d.roedores?.productoId)}${senasaSelectResolucion('roedores.resolucionId', 'Resolución prefijada', d.roedores?.resolucionId)}${senasaInput('roedores.principioActivo', 'Principio activo', 'text', d.roedores?.principioActivo)}${senasaInput('roedores.concentracion', 'Concentración', 'text', d.roedores?.concentracion)}${senasaInput('roedores.habilitacionHabitual', 'Habilitación habitual', 'text', d.roedores?.habilitacionHabitual)}${senasaInput('roedores.resolucionSenasa', 'Resolución SENASA', 'text', d.roedores?.resolucionSenasa)}${senasaInput('roedores.fechaResolucionSenasa', 'Fecha resolución', 'date', d.roedores?.fechaResolucionSenasa)}${senasaInput('roedores.frecuenciaVerificacion', 'Frecuencia verificación/reposición', 'text', d.roedores?.frecuenciaVerificacion)}${senasaInput('roedores.sectoresGrupo1', 'Sectores letras grupo 1', 'text', d.roedores?.sectoresGrupo1)}${senasaInput('roedores.frecuenciaGrupo1', 'Frecuencia grupo 1', 'text', d.roedores?.frecuenciaGrupo1)}${senasaInput('roedores.sectoresGrupo2', 'Sectores letras grupo 2', 'text', d.roedores?.sectoresGrupo2)}${senasaInput('roedores.frecuenciaGrupo2', 'Frecuencia grupo 2', 'text', d.roedores?.frecuenciaGrupo2)}
    </div>
    <h4>Insectos — sectores externos</h4><div class="senasa-grid-tres">${senasaInput('insectosExternos.periodoDesde', 'Periodo desde', 'date', d.insectosExternos?.periodoDesde || d.periodoDesde)}${senasaInput('insectosExternos.periodoHasta', 'Periodo hasta', 'date', d.insectosExternos?.periodoHasta || d.periodoHasta)}${senasaSelectProducto('insectosExternos.productoId', 'Producto insecticida exterior', d.insectosExternos?.productoId)}${senasaSelectResolucion('insectosExternos.resolucionId', 'Resolución prefijada', d.insectosExternos?.resolucionId)}${senasaInput('insectosExternos.principioActivo', 'Principio activo', 'text', d.insectosExternos?.principioActivo)}${senasaInput('insectosExternos.concentracion', 'Concentración', 'text', d.insectosExternos?.concentracion)}${senasaInput('insectosExternos.habilitacionHabitual', 'Habilitación habitual', 'text', d.insectosExternos?.habilitacionHabitual)}${senasaInput('insectosExternos.resolucionSenasa', 'Resolución SENASA', 'text', d.insectosExternos?.resolucionSenasa)}${senasaInput('insectosExternos.fechaResolucionSenasa', 'Fecha resolución', 'date', d.insectosExternos?.fechaResolucionSenasa)}${senasaInput('insectosExternos.frecuenciaHoras', 'Frecuencia en horas', 'text', d.insectosExternos?.frecuenciaHoras)}${senasaInput('insectosExternos.sectoresGrupo1', 'Sectores grupo 1', 'text', d.insectosExternos?.sectoresGrupo1)}${senasaInput('insectosExternos.sectoresGrupo2', 'Sectores grupo 2', 'text', d.insectosExternos?.sectoresGrupo2)}</div>
    <h4>Insectos — sectores internos</h4><div class="senasa-grid-tres">${senasaInput('insectosInternos.periodoDesde', 'Periodo desde', 'date', d.insectosInternos?.periodoDesde || d.periodoDesde)}${senasaInput('insectosInternos.periodoHasta', 'Periodo hasta', 'date', d.insectosInternos?.periodoHasta || d.periodoHasta)}${senasaInput('insectosInternos.colorSeccionPlano', 'Color/sección del plano', 'text', d.insectosInternos?.colorSeccionPlano)}${senasaSelectProducto('insectosInternos.productoId', 'Producto insecticida interior', d.insectosInternos?.productoId)}${senasaSelectResolucion('insectosInternos.resolucionId', 'Resolución prefijada', d.insectosInternos?.resolucionId)}${senasaInput('insectosInternos.principioActivo', 'Principio activo', 'text', d.insectosInternos?.principioActivo)}${senasaInput('insectosInternos.concentracion', 'Concentración', 'text', d.insectosInternos?.concentracion)}${senasaInput('insectosInternos.habilitacionHabitual', 'Habilitación habitual', 'text', d.insectosInternos?.habilitacionHabitual)}${senasaInput('insectosInternos.resolucionSenasa', 'Resolución SENASA', 'text', d.insectosInternos?.resolucionSenasa)}${senasaInput('insectosInternos.fechaResolucionSenasa', 'Fecha resolución', 'date', d.insectosInternos?.fechaResolucionSenasa)}${senasaInput('insectosInternos.dia', 'Día', 'date', d.insectosInternos?.dia)}${senasaInput('insectosInternos.hora', 'Hora', 'time', d.insectosInternos?.hora)}${senasaInput('insectosInternos.tiempoActuacionHoras', 'Tiempo actuación en horas', 'text', d.insectosInternos?.tiempoActuacionHoras)}</div><p class="muted">Texto fijo: “Para proceder luego al lavado y desinfección de la sección”.</p>
    <h4>Otras plagas</h4><div class="senasa-grid-tres">${senasaInput('otrasPlagas.especiesVoladoras', 'Especies voladoras', 'text', d.otrasPlagas?.especiesVoladoras)}${senasaInput('otrasPlagas.especiesCaminadoras', 'Especies caminadoras', 'text', d.otrasPlagas?.especiesCaminadoras)}${senasaInput('otrasPlagas.metodologia', 'Metodología/sistema de control', 'text', d.otrasPlagas?.metodologia)}${senasaInput('otrasPlagas.frecuencia', 'Frecuencia', 'text', d.otrasPlagas?.frecuencia)}${senasaInput('otrasPlagas.sectores', 'Sectores', 'text', d.otrasPlagas?.sectores)}</div>
  ...
  form.innerHTML = `<div class="senasa-grid-tres"><label>Tipo documento<select name="tipoDocumento"><option value="AVISO_MIP" ${esAviso ? 'selected' : ''}>Planilla de Aviso — Programa de Actividades MIP</option><option value="INFORME_CONTROL_PLAGAS" ${!esAviso ? 'selected' : ''}>Informe — Control de Plagas</option></select></label>${senasaInput('numeroCircular', 'Circular Nº', 'text', d.numeroCircular)}${senasaInput('fechaRecepcion', 'Fecha de recepción', 'date', d.fechaRecepcion)}${senasaInput('periodoDesde', 'Periodo desde', 'date', d.periodoDesde)}${senasaInput('periodoHasta', 'Periodo hasta', 'date', d.periodoHasta)}${senasaInput('fechaActividad', 'Fecha de actividad', 'date', d.fechaActividad)}${senasaInput('horaActividad', 'Hora de actividad', 'time', d.horaActividad)}<label>Cliente / establecimiento<select name="clienteId" data-senasa-cliente>${clienteOptions}</select></label></div>
  <h4>Datos del establecimiento / cliente</h4><div class="senasa-grid-tres">${senasaInput('cliente.nombre', 'Razón social / nombre', 'text', cliente.nombre)}${senasaInput('cliente.domicilio', 'Domicilio', 'text', cliente.domicilio)}${senasaInput('cliente.telefono', 'Teléfono', 'text', cliente.telefono)}${senasaInput('cliente.localidad', 'Localidad', 'text', cliente.localidad)}${senasaInput('cliente.provincia', 'Provincia', 'text', cliente.provincia)}${senasaInput('cliente.cuitDni', 'CUIT/DNI', 'text', cliente.cuitDni)}${senasaInput('establecimiento.establecimientoOficial', 'Establecimiento Nº Oficial', 'text', est.establecimientoOficial)}${senasaInput('establecimiento.supervisor', 'Supervisor', 'text', est.supervisor)}${senasaInput('establecimiento.responsableSiv', 'Responsable por S.I.V.', 'text', est.responsableSiv)}${senasaInput('establecimiento.departamentoPartido', 'Departamento / Partido', 'text', est.departamentoPartido)}${senasaTextarea('cliente.observaciones', 'Observaciones', cliente.observaciones)}</div>${esAviso ? avisoHtml : informeHtml}`;
...
function recogerSenasaForm() {
  ...
  const base = { ...senasaDocumentoActual, roedores: { filas: [] }, otrasPlagas: { filas: [] }, areasExternas: { filas: [] }, hermeticidad: { filas: [] } };
...
function senasaLinea(label, value) { return `<p><strong>${label}</strong> ${escapeHtmlClient(value || '....................')}</p>`; }
function renderSenasaPreview() {
  ...
  preview.innerHTML = `<div class="senasa-doc-title"><h2>${titulo}</h2><h3>${subtitulo}</h3><b>CIRCULAR Nº ${escapeHtmlClient(d.numeroCircular || '........')}</b></div><p class="senasa-fecha">FECHA DE RECEPCIÓN: ${escapeHtmlClient(d.fechaRecepcion || '__/__/20__')}</p><h4>ESTABLECIMIENTO</h4><div class="senasa-box"><p>Establecimiento Nº Oficial <b>${escapeHtmlClient(est.establecimientoOficial || '')}</b> &nbsp; Razón Social <b>${escapeHtmlClient(cliente.nombre || '')}</b></p><p>Domicilio: <b>${escapeHtmlClient(cliente.domicilio || '')}</b> &nbsp; Tel/Fax: <b>${escapeHtmlClient(cliente.telefono || '')}</b></p><p>Localidad: <b>${escapeHtmlClient(cliente.localidad || '')}</b> &nbsp; Dpto/Partido: <b>${escapeHtmlClient(est.departamentoPartido || '')}</b> &nbsp; Provincia: <b>${escapeHtmlClient(cliente.provincia || '')}</b></p><p>Supervisor: <b>${escapeHtmlClient(est.supervisor || '')}</b> &nbsp; Responsable por el S.I.V. <b>${escapeHtmlClient(est.responsableSiv || '')}</b></p></div>${esAviso ? `<h4>ROEDORES</h4><div class="senasa-box">${senasaLinea('Periodo comprendido entre', `${d.roedores?.periodoDesde || d.periodoDesde || ''} y ${d.roedores?.periodoHasta || d.periodoHasta || ''}`)}${senasaLinea('Producto', d.roedores?.productoNombre)}${senasaLinea('Principio activo', d.roedores?.principioActivo)}${senasaLinea('Concentración', d.roedores?.concentracion)}${senasaLinea('Habilitación', d.roedores?.habilitacionHabitual || d.roedores?.resolucionSenasa)}${senasaLinea('Frecuencia y sectores', `${d.roedores?.frecuenciaVerificacion || ''} · ${d.roedores?.sectoresGrupo1 || ''} · ${d.roedores?.sectoresGrupo2 || ''}`)}</div><h4>INSECTOS</h4><div class="senasa-box">${senasaLinea('Producto externo', d.insectosExternos?.productoNombre)}${senasaLinea('Principio activo externo', d.insectosExternos?.principioActivo)}${senasaLinea('Concentración externa', d.insectosExternos?.concentracion)}${senasaLinea('Habilitación externa', d.insectosExternos?.habilitacionHabitual || d.insectosExternos?.resolucionSenasa)}${senasaLinea('Producto interno', d.insectosInternos?.productoNombre)}${senasaLinea('Principio activo interno', d.insectosInternos?.principioActivo)}${senasaLinea('Concentración interna', d.insectosInternos?.concentracion)}${senasaLinea('Habilitación interna', d.insectosInternos?.habilitacionHabitual || d.insectosInternos?.resolucionSenasa)}<p>Para proceder luego al lavado y desinfección de la sección</p></div><h4>OTRAS PLAGAS / ÁREAS / HERMETICIDAD</h4><div class="senasa-box">${senasaLinea('Otras plagas', `${d.otrasPlagas?.especiesVoladoras || ''} ${d.otrasPlagas?.especiesCaminadoras || ''}`)}${senasaLinea('Áreas externas', d.areasExternas?.sectores)}${senasaLinea('Hermeticidad', d.hermeticidad?.sectores)}</div>` : `<h4>INFORME CORRESPONDIENTE PROGRAMA DE ACTIVIDADES MIP DE FECHA: ${escapeHtmlClient(d.fechaProgramaMip || '')}</h4><h4>ROEDORES</h4>${roedoresTabla}<h4>INSECTOS</h4><div class="senasa-box">${senasaLinea('Sectores externos', `${d.insectosExternos?.fecha || ''} ${d.insectosExternos?.hora || ''} · ${d.insectosExternos?.sectoresRecorridos || ''}`)}${senasaLinea('Sectores internos', `${d.insectosInternos?.fecha || ''} ${d.insectosInternos?.hora || ''} · Trampa Nº ${d.insectosInternos?.trampaNumero || ''}`)}</div><h4>VERIFICACIÓN</h4><div class="senasa-box">${senasaLinea('Acompañamiento', `${d.verificacion?.fechaAcompanamiento || ''} de ${d.verificacion?.horaDesde || ''} a ${d.verificacion?.horaHasta || ''}`)}${senasaLinea('Incrementar actividades', d.verificacion?.incrementarActividades)}${senasaLinea('Observaciones finales', d.verificacion?.observacionesFinales)}</div>`}<div class="senasa-firmas"><span>Responsable Técnico MIP</span><span>Recibido SIV</span></div>`;
...
$('#senasa-btn-pdf')?.addEventListener('click', async () => { if (!senasaDocumentoActual?.id) await guardarSenasaDocumento(false); if (senasaDocumentoActual?.id) window.open(`/api/senasa/documentos/${senasaDocumentoActual.id}/pdf`, '_blank', 'noopener'); });
...
$('#senasa-form')?.addEventListener('input', (e) => { if (e.target?.matches('[name]')) renderSenasaPreview(); });
$('#senasa-form')?.addEventListener('change', (e) => {
  ...
  if (prodSel) { const basePath = prodSel.name.replace(/\.productoId$/, ''); Object.assign(senasaDocumentoActual[basePath] ||= {}, senasaProductoTecnico(prodSel.value), { productoId: prodSel.value }); renderSenasaForm(); return; }
  ...
  if (e.target.name === 'tipoDocumento') { senasaDocumentoActual.tipoDocumento = e.target.value; renderSenasaForm(); return; }
  renderSenasaPreview();
});

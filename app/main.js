...
function senasaEtiquetaRecomendada(label) {
  return `<span class="senasa-label-row"><span>${escapeHtmlClient(label)}</span><span class="senasa-recomendado">Recomendado</span></span>`;
}
...
function senasaMiniCard(titulo, textoGuia, html) {
  return `<article class="senasa-mini-card"><div class="senasa-mini-card-head"><h5>${escapeHtmlClient(titulo)}</h5>${textoGuia ? `<p>${escapeHtmlClient(textoGuia)}</p>` : ''}</div>${html}</article>`;
}
function senasaAyudaBloque(tipo) {
  const ayudas = {
    establecimiento: 'Elegí el cliente para traer datos guardados y revisá razón social, domicilio y responsable antes de continuar.',
    roedores: 'Frecuencia y letras ya vienen precargadas; solo cambialas si hace falta para este establecimiento.',
    insectosExternos: 'Usá este bloque para sectores exteriores. Primero seleccioná el producto: los datos técnicos se completan solos.',
    insectosInternos: 'Indicá sectores internos, fechas y horarios si corresponden. Los campos vacíos no bloquean la carga.',
    otrasPlagas: 'Separá voladoras y caminadoras para que la lectura sea clara y no se mezclen sectores.',
    areasExternas: 'Registrá mantenimiento, orden y sectores externos recorridos sin modificar la estructura del documento.',
    hermeticidad: 'Anotá sectores o elementos evaluados y observaciones prácticas para la revisión final.',
    informeRoedores: 'Cargá solo hallazgos reales de cada casilla; si no hubo actividad podés dejar observaciones simples.',
    informeOtras: 'Registrá actividad real por sector para voladoras y caminadoras.',
    informeAreas: 'Usá una fila por novedad externa encontrada durante la recorrida.',
    informeHermeticidad: 'Usá una fila por elemento verificado para que el seguimiento sea fácil de leer.'
  };
  return `<p class="senasa-ayuda-bloque">${escapeHtmlClient(ayudas[tipo] || 'Completá este bloque con los datos disponibles; nada nuevo se vuelve obligatorio.')}</p>`;
}
function senasaPasoIndicador(paso, total, texto) {
  return `<div class="senasa-paso-indicador"><span>Paso ${paso} de ${total}</span><strong>${escapeHtmlClient(texto)}</strong></div>`;
}
...
function senasaValorRegulatorio(value = '') {
  const text = senasaValor(value, '');
...
function senasaRegistroValor(datos = {}, fallback = SENASA_TEXTO_PENDIENTE) {
  ...
  const numero = senasaValorRegulatorio(producto.numeroRegistro);
  if (numero) {
    const tipo = senasaValorRegulatorio(producto.tipoRegistro);
    const yaIncluyeTipo = tipo && numero.toLowerCase().startsWith(tipo.toLowerCase());
    return senasaValor([!yaIncluyeTipo ? tipo : '', numero].filter(Boolean).join(' '), fallback);
  }
...
function renderSenasaListas() {
  ...
  $('#senasa-resoluciones').innerHTML = '<div class="item">Los datos regulatorios se mantienen dentro del producto SENASA / ANMAT seleccionado. Use la tabla de productos para revisar cada ficha; no hay carga manual de resoluciones sueltas.</div>';
  const tabla = $('#senasa-productos-tabla');
  if (tabla) {
    const rows = senasaProductos.filter(productoEsSenasaMip).map((p) => `<tr><td>${escapeHtmlClient(senasaValor(p.nombre || p.nombreComercial))}</td><td>${escapeHtmlClient(senasaValor(p.principioActivo))}</td><td>${escapeHtmlClient(senasaValor(p.concentracion))}</td><td>${escapeHtmlClient(senasaRegistroValor(p))}</td><td>${escapeHtmlClient(senasaValor(p.disposicionRegistro))}</td><td>${escapeHtmlClient(senasaProductoEstado({ productoNombre: p.nombre || p.nombreComercial, ...p }))}</td><td><button type="button" data-senasa-producto-detalle="${p.id}">Ver</button></td></tr>`).join('');
    tabla.innerHTML = `<div class="item">Productos oficiales precargados: al seleccionarlos, el formulario autocompleta los datos regulatorios y los muestra de solo lectura.</div><div class="senasa-table-wrap"><table class="senasa-productos-table"><thead><tr><th>Producto comercial</th><th>Principio activo</th><th>Concentración</th><th>Registro</th><th>Disposición</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>${rows || '<tr><td colspan="7">Sin productos SENASA.</td></tr>'}</tbody></table></div>`;
  }
}
...
function senasaProductoPrevistoDesdeProductoId(productoId) {
  ...
  const item = { ...tecnico, id: `${tecnico.productoId}-${Date.now()}`, areaSector: '', frecuencia: '', metodo: '' };
...
function senasaProductosPrevistosDocumento(d = {}) {
  const seccionesOriginales = [d.roedores, d.insectosExternos, d.insectosInternos, d.otrasPlagas?.voladoras || d.otrasPlagas, d.otrasPlagas?.caminadoras]
    .filter((item) => senasaProductoDatos(item || {}).nombre)
    .map((item) => ({ ...item, habilitacionCompleta: senasaHabilitacionCompleta(item) }));
  if (d.tipoDocumento === 'AVISO_MIP' || seccionesOriginales.length) return seccionesOriginales;
  ...
  return [d.roedores, d.insectosExternos, d.insectosInternos, d.otrasPlagas, d.otrasPlagas?.voladoras, d.otrasPlagas?.caminadoras]
    .filter((item) => senasaProductoDatos(item || {}).nombre)
    .map((item) => ({ ...item, habilitacionCompleta: senasaHabilitacionCompleta(item) }));
}
function renderSenasaProductosPrevistosTabla(d = {}) {
  const productos = senasaProductosPrevistosDocumento(d);
  if (!productos.length) return '<div class="senasa-empty-box">No hay productos a aplicar. Seleccione un producto MIP y presione “Agregar producto”.</div>';
  const rows = productos.map((item, i) => {
    const producto = senasaProductoDatos(item);
    return `<tr><td>${escapeHtmlClient(senasaValor(producto.nombre))}</td><td>${escapeHtmlClient(senasaValor(producto.principioActivo))}</td><td>${escapeHtmlClient(senasaValor(producto.concentracion))}</td><td>${escapeHtmlClient(senasaValor(senasaHabilitacionCompleta(item)))}</td><td>${escapeHtmlClient(senasaRegistroValor(item))}</td><td>${escapeHtmlClient(senasaValor(producto.disposicionRegistro))}</td><td><input name="productosPrevistos.${i}.areaSector" value="${escapeHtmlClient(item.areaSector || '')}" placeholder="Área / sector"></td><td><input name="productosPrevistos.${i}.frecuencia" value="${escapeHtmlClient(item.frecuencia || '')}" placeholder="Frecuencia"></td><td><input name="productosPrevistos.${i}.metodo" value="${escapeHtmlClient(item.metodo || '')}" placeholder="Método"></td><td><button type="button" data-senasa-producto-previsto-remove="${i}">Quitar</button></td></tr>`;
  }).join('');
  return `<div class="senasa-table-wrap"><table class="senasa-productos-table"><thead><tr><th>Producto</th><th>Principio activo</th><th>Concentración</th><th>Habilitación</th><th>Registro / Resolución</th><th>Disposición</th><th>Área / sector</th><th>Frecuencia</th><th>Método</th><th></th></tr></thead><tbody>${rows}</tbody></table></div>`;
}
function senasaWizardNavHtml(active = 1) {
  const pasos = [['1', 'Establecimiento'], ['2', 'Periodo'], ['3', 'Productos'], ['4', 'PDF']];
...
function senasaMetodoSelect(name, value = '') {
  const opciones = ['', 'Pulverización', 'Cebado', 'Nebulización', 'Control de cajas cebaderas', 'Reposición de cebos', 'Monitoreo', 'Inspección'];
  return `<select name="${name}">${opciones.map((op) => `<option value="${escapeHtmlClient(op)}" ${String(value || '') === op ? 'selected' : ''}>${escapeHtmlClient(op || 'Seleccione metodología')}</option>`).join('')}</select>`;
}
function senasaSeccionProductoCampos(key, titulo, seccion = {}, ayudaTipo = '') {
  const producto = senasaProductoDatos(seccion);
  return `<div class="senasa-original-edit-block">
    <h4>${escapeHtmlClient(titulo)}</h4>
    ${senasaAyudaBloque(ayudaTipo || key.split('.').at(0))}
    <div class="senasa-mini-card-grid">
      ${senasaMiniCard('Fechas', 'Si el periodo general ya está cargado, estas fechas se completan como referencia.', `<div class="senasa-grid-dos">
        ${senasaInput(`${key}.periodoDesde`, senasaEtiquetaRecomendada('Periodo desde'), 'date', seccion.periodoDesde || (senasaDocumentoActual?.periodoDesde || ''))}
        ${senasaInput(`${key}.periodoHasta`, senasaEtiquetaRecomendada('Periodo hasta'), 'date', seccion.periodoHasta || (senasaDocumentoActual?.periodoHasta || ''))}
      </div>`)}
      ${senasaMiniCard('Producto', 'Primero seleccioná el producto. Los datos técnicos se completan solos.', `<div class="senasa-grid-dos">
        <label>${senasaEtiquetaRecomendada('Metodología / sistema')} ${senasaMetodoSelect(`${key}.metodologia`, seccion.metodologia || seccion.metodo)}</label>
        ${senasaSelectProducto(`${key}.productoId`, senasaEtiquetaRecomendada('Producto a emplear'), seccion.productoId, seccion)}
      ...
      ${senasaMiniCard('Frecuencia y sectores', 'Frecuencia y letras ya vienen precargadas, solo cambialas si hace falta.', `<div class="senasa-grid-dos">
        ${senasaInput(`${key}.frecuenciaGrupo1`, senasaEtiquetaRecomendada('Frecuencia'), 'text', seccion.frecuenciaGrupo1 || seccion.frecuenciaVerificacion || seccion.frecuenciaHoras || seccion.frecuencia)}
        ${senasaTextarea(`${key}.sectoresGrupo1`, senasaEtiquetaRecomendada('Sectores grupo 1 / letras'), seccion.sectoresGrupo1)}
        ${senasaInput(`${key}.frecuenciaGrupo2`, 'Frecuencia grupo 2', 'text', seccion.frecuenciaGrupo2)}
        ${senasaTextarea(`${key}.sectoresGrupo2`, 'Sectores grupo 2 / letras', seccion.sectoresGrupo2)}
      </div>`)}
      ${senasaMiniCard('Observaciones', 'Sumá aclaraciones breves si necesitás dejar contexto para quien revisa.', `<div class="senasa-grid-dos">
        ${senasaTextarea(`${key}.observacionesSectores`, 'Observaciones de sectores', seccion.observacionesSectores || seccion.observaciones)}
      </div>`)}
    </div>
  </div>`;
}
function senasaAvisoPlantillaOriginalHtml(d = {}) {
  ...
  const cabecera = `<h4>PROGRAMA DE ACTIVIDADES MIP</h4>
    ${senasaPasoIndicador(1, 5, 'Datos base del aviso')}
    <div class="senasa-mini-card-grid">
      ${senasaMiniCard('Fechas', 'Cargá el periodo general una vez. Las secciones pueden tomar estas fechas como referencia.', `<div class="senasa-grid-tres">
        <label>Tipo documento<select name="tipoDocumento"><option value="AVISO_MIP" selected>Planilla de Aviso — Programa de Actividades MIP</option><option value="INFORME_CONTROL_PLAGAS">Informe — Control de Plagas</option></select></label>
        ${senasaInput('numeroCircular', 'Circular Nº', 'text', d.numeroCircular)}
        ${senasaInput('fechaRecepcion', senasaEtiquetaRecomendada('Fecha de recepción'), 'date', d.fechaRecepcion)}
        ${senasaInput('periodoDesde', senasaEtiquetaRecomendada('Periodo general desde'), 'date', d.periodoDesde)}
        ${senasaInput('periodoHasta', senasaEtiquetaRecomendada('Periodo general hasta'), 'date', d.periodoHasta)}
        <label>${senasaEtiquetaRecomendada('Cliente / establecimiento')}<select name="clienteId" data-senasa-cliente>${clienteOptions}</select></label>
      </div>`)}
    </div>`;
  const establecimiento = `${senasaPasoIndicador(2, 5, 'Establecimiento')}
    ${senasaAyudaBloque('establecimiento')}
    <div class="senasa-mini-card-grid">
      ${senasaMiniCard('Establecimiento', 'Revisá estos datos luego de seleccionar el cliente; podés ajustarlos sin cambiar datos guardados existentes.', `<div class="senasa-grid-tres">
        ${senasaInput('establecimiento.establecimientoOficial', 'Establecimiento Nº Oficial', 'text', est.establecimientoOficial)}
        ${senasaInput('cliente.nombre', senasaEtiquetaRecomendada('Razón social'), 'text', cliente.nombre)}
        ${senasaInput('cliente.domicilio', senasaEtiquetaRecomendada('Domicilio'), 'text', cliente.domicilio)}
        ${senasaInput('cliente.telefono', 'Tel/Fax', 'text', cliente.telefono)}
        ${senasaInput('cliente.localidad', 'Localidad', 'text', cliente.localidad)}
        ${senasaInput('establecimiento.departamentoPartido', 'Dpto/Partido', 'text', est.departamentoPartido)}
        ${senasaInput('cliente.provincia', 'Provincia', 'text', cliente.provincia)}
        ${senasaInput('establecimiento.supervisor', 'Supervisor', 'text', est.supervisor)}
        ${senasaInput('establecimiento.responsableSiv', senasaEtiquetaRecomendada('Responsable por el S.I.V.'), 'text', est.responsableSiv)}
      </div>`)}
    </div>`;
  const insectos = `${senasaPasoIndicador(4, 5, 'Insectos')}
    ${senasaSeccionProductoCampos('insectosExternos', 'Sectores externos', d.insectosExternos || {}, 'insectosExternos')}
    ${senasaSeccionProductoCampos('insectosInternos', 'Sectores internos', d.insectosInternos || {}, 'insectosInternos')}
    ${senasaMiniCard('Observaciones', 'Si corresponde, completá día, hora y lapso de actuación interna.', `<div class="senasa-grid-tres">
      ${senasaInput('insectosInternos.diaActividad', 'Día de actividad interna', 'date', d.insectosInternos?.diaActividad)}
      ${senasaInput('insectosInternos.horaActividad', 'Hora actividad interna', 'time', d.insectosInternos?.horaActividad)}
      ${senasaInput('insectosInternos.lapsoHoras', 'Lapso de actuación (horas)', 'text', d.insectosInternos?.lapsoHoras)}
    </div>`)} `;
  const otrasPlagas = `${senasaPasoIndicador(4, 5, 'Otras plagas')}
    ${senasaSeccionProductoCampos('otrasPlagas.voladoras', 'Sectores externos especies vivas', d.otrasPlagas?.voladoras || {}, 'otrasPlagas')}
    ${senasaSeccionProductoCampos('otrasPlagas.caminadoras', 'Sectores externos especies caminadoras', d.otrasPlagas?.caminadoras || {}, 'otrasPlagas')}`;
  const areasExternas = `${senasaPasoIndicador(4, 5, 'Áreas externas')}
    ${senasaAyudaBloque('areasExternas')}
    <div class="senasa-mini-card-grid">
      ${senasaMiniCard('Fechas', 'Usá el periodo general salvo que este bloque tenga una fecha distinta.', `<div class="senasa-grid-dos">
        ${senasaInput('areasExternas.periodoDesde', senasaEtiquetaRecomendada('Periodo desde'), 'date', d.areasExternas?.periodoDesde || d.periodoDesde)}
        ${senasaInput('areasExternas.periodoHasta', senasaEtiquetaRecomendada('Periodo hasta'), 'date', d.areasExternas?.periodoHasta || d.periodoHasta)}
      </div>`)}
      ${senasaMiniCard('Frecuencia y sectores', 'Frecuencia y letras ya vienen precargadas, solo cambialas si hace falta.', `<div class="senasa-grid-dos">
        ${senasaInput('areasExternas.frecuenciaGrupo1', senasaEtiquetaRecomendada('Frecuencia'), 'text', d.areasExternas?.frecuenciaGrupo1 || d.areasExternas?.frecuencia)}
        ${senasaTextarea('areasExternas.sectoresGrupo1', senasaEtiquetaRecomendada('Sectores / letras'), d.areasExternas?.sectoresGrupo1 || d.areasExternas?.sectoresMantenidos)}
      </div>`)}
      ${senasaMiniCard('Observaciones', 'Agregá detalles de mantenimiento, orden y limpieza si aplica.', `${senasaTextarea('areasExternas.observaciones', 'Observaciones', d.areasExternas?.observaciones)}`)}
    </div>`;
  const hermeticidad = `${senasaPasoIndicador(4, 5, 'Hermeticidad')}
    ${senasaAyudaBloque('hermeticidad')}
    <div class="senasa-mini-card-grid">
      ${senasaMiniCard('Fechas', 'Usá el periodo general salvo que este bloque tenga una fecha distinta.', `<div class="senasa-grid-dos">
        ${senasaInput('hermeticidad.periodoDesde', senasaEtiquetaRecomendada('Periodo desde'), 'date', d.hermeticidad?.periodoDesde || d.periodoDesde)}
        ${senasaInput('hermeticidad.periodoHasta', senasaEtiquetaRecomendada('Periodo hasta'), 'date', d.hermeticidad?.periodoHasta || d.periodoHasta)}
      </div>`)}
      ${senasaMiniCard('Frecuencia y sectores', 'Frecuencia y letras ya vienen precargadas, solo cambialas si hace falta.', `<div class="senasa-grid-dos">
        ${senasaInput('hermeticidad.frecuenciaGrupo1', senasaEtiquetaRecomendada('Frecuencia'), 'text', d.hermeticidad?.frecuenciaGrupo1 || d.hermeticidad?.frecuencia)}
        ${senasaTextarea('hermeticidad.sectoresGrupo1', senasaEtiquetaRecomendada('Sectores / letras'), d.hermeticidad?.sectoresGrupo1 || d.hermeticidad?.sectoresEvaluados)}
      </div>`)}
      ${senasaMiniCard('Observaciones', 'Anotá elementos evaluados o aclaraciones prácticas.', `${senasaTextarea('hermeticidad.observaciones', 'Elementos / observaciones', d.hermeticidad?.observaciones)}`)}
    </div>`;
  return `<div class="senasa-alerta-estructural senasa-alerta-plan">Carga guiada: las secciones siguen plegables y el PDF no cambia. Los campos marcados como “Recomendado” ayudan a revisar, pero no bloquean.</div>
    ${senasaFieldset('PLANILLA DE AVISO', cabecera, true)}
    ${senasaFieldset('ESTABLECIMIENTO', establecimiento, true)}
    ${senasaFieldset('ROEDORES', `${senasaPasoIndicador(3, 5, 'Roedores')}${senasaSeccionProductoCampos('roedores', 'Roedores', d.roedores || {}, 'roedores')}`, true)}
    ${senasaFieldset('INSECTOS', insectos, true)}
    ${senasaFieldset('OTRAS PLAGAS', otrasPlagas, true)}
    ${senasaFieldset('ÁREAS EXTERNAS Y ESPACIOS VERDES', areasExternas, false)}
    ${senasaFieldset('HERMETICIDAD', hermeticidad, false)}
    ${senasaFieldset('FIRMAS', `${senasaPasoIndicador(5, 5, 'Revisión final y PDF')}<p class="senasa-ayuda-bloque">Guardá cuando termines y usá Ver PDF o Descargar PDF para revisar el documento final.</p><div class="senasa-firmas senasa-firmas-edit"><span>Responsable Técnico MIP</span><span>Recibido SIV</span></div>`, false)}`;
}
...
function renderSenasaForm() {
  ...
  const planificacionHtml = `<div class="senasa-grid-tres senasa-step-grid">${senasaInput('periodoDesde', 'Periodo desde', 'date', d.periodoDesde)}${senasaInput('periodoHasta', 'Periodo hasta', 'date', d.periodoHasta)}</div>`;
  const productoSeleccionado = senasaProductoPrevistoSeleccionado ? senasaProductoTecnico(senasaProductoPrevistoSeleccionado) : {};
  const productosHtml = `<div class="senasa-productos-asistente"><div class="senasa-producto-add-card"><label>Selector de producto MIP<select data-senasa-producto-previsto-select><option value="">Seleccione producto MIP registrado</option>${productosOptions.replace('<option value="">Seleccione producto MIP registrado</option>', '')}</select></label><button type="button" class="btn-primary" data-senasa-producto-previsto-add>Agregar producto</button></div>${senasaProductoResumenHtml(productoSeleccionado)}<h4>Productos a aplicar</h4>${renderSenasaProductosPrevistosTabla(d)}</div>`;
...
function renderSenasaTablaRoedores(rows) {
  ...
  return `<h4>Roedores</h4>${senasaAyudaBloque('informeRoedores')}<table class="senasa-edit-table"><thead><tr><th>Casilla Nº</th><th>Vivos</th><th>Muertos</th><th>Materia fecal</th><th>Consumo cebo</th><th>Observaciones</th><th>Medida correctiva</th><th></th></tr></thead><tbody>${trs}</tbody></table><button type="button" data-senasa-row-add="roedores">Agregar casilla</button>`;
}
...
function renderSenasaTablaSimple(key, title, rows, opciones) {
  ...
  return `<h4>${title}</h4>${senasaAyudaBloque(key === 'hermeticidad' ? 'informeHermeticidad' : 'informeAreas')}<table class="senasa-edit-table"><thead><tr><th>Sector</th><th>${key === 'hermeticidad' ? 'Elemento verificado' : 'Tipo de novedad'}</th><th>Observaciones</th><th>Medida correctiva</th><th></th></tr></thead><tbody>${trs}</tbody></table><button type="button" data-senasa-row-add="${key}">Agregar fila</button>`;
}
...
function senasaHidratarProductosDocumento(documento = {}) {
  ...
  if (documento.otrasPlagas?.voladoras) senasaHidratarProductoSeleccionado(documento.otrasPlagas.voladoras);
  if (documento.otrasPlagas?.caminadoras) senasaHidratarProductoSeleccionado(documento.otrasPlagas.caminadoras);
  return documento;
}
...
function senasaSincronizarProductosDocumento(documento = {}) {
  ...
  if (documento.otrasPlagas?.voladoras) senasaSincronizarProductoSeccion(documento.otrasPlagas.voladoras);
  if (documento.otrasPlagas?.caminadoras) senasaSincronizarProductoSeccion(documento.otrasPlagas.caminadoras);
  return documento;
}
...
function senasaProductoPreviewHtml(titulo, datos = {}) {
  const producto = senasaProductoDatos(datos);
  return `<div class="senasa-box senasa-producto-doc"><h5>${escapeHtmlClient(titulo)}</h5>${senasaLinea('Producto', producto.nombre)}${senasaLinea('Principio activo', producto.principioActivo)}${senasaLinea('Concentración', producto.concentracion)}${senasaLinea('Organismo', producto.habilitacionHabitual || producto.organismoHabilitante || producto.organismoRegulador)}${senasaLinea('RNPUD / Registro', senasaRegistroValor(producto), SENASA_REGISTRO_PENDIENTE)}${senasaLinea('Disposición', producto.disposicionRegistro, SENASA_REGISTRO_PENDIENTE)}${senasaLinea('Vencimiento', producto.fechaVencimientoRegistro, SENASA_REGISTRO_PENDIENTE)}${senasaLinea('Titular', producto.empresaTitularRegistro, SENASA_REGISTRO_PENDIENTE)}</div>`;
}
function senasaProductosRegulatoriosHtml(d = {}) {
  const productos = senasaProductosPrevistosDocumento(d);
  if (!productos.length) return '<div class="senasa-empty-box">No hay productos MIP seleccionados.</div>';
  const rows = productos.map((item) => {
    const producto = senasaProductoDatos(item);
    return `<tr><td>${escapeHtmlClient(senasaValor(producto.nombre))}</td><td>${escapeHtmlClient(senasaValor(producto.principioActivo))}</td><td>${escapeHtmlClient(senasaValor(producto.concentracion))}</td><td>${escapeHtmlClient(senasaValor(producto.habilitacionHabitual || producto.organismoHabilitante || producto.organismoRegulador))}</td><td>${escapeHtmlClient(senasaRegistroValor(producto))}</td><td>${escapeHtmlClient(senasaValor(producto.disposicionRegistro))}</td><td>${escapeHtmlClient(senasaValor(producto.fechaVencimientoRegistro))}</td><td>${escapeHtmlClient(senasaValor(producto.empresaTitularRegistro))}</td></tr>`;
  }).join('');
  return `<div class="senasa-table-wrap"><table class="senasa-productos-table"><thead><tr><th>Producto</th><th>Principio activo</th><th>Concentración</th><th>Organismo</th><th>RNPUD / Registro</th><th>Disposición</th><th>Vencimiento</th><th>Titular</th></tr></thead><tbody>${rows}</tbody></table></div>`;
}
...
function senasaPuntos(value, fallback = '........................') {
  return escapeHtmlClient(senasaValor(value, fallback));
}
function senasaPeriodoTexto(seccion = {}, d = {}) {
  return `${senasaPuntos(seccion.periodoDesde || d.periodoDesde)} y el ${senasaPuntos(seccion.periodoHasta || d.periodoHasta)}`;
}
function senasaProductoTexto(seccion = {}) {
  const producto = senasaProductoDatos(seccion);
  return {
    nombre: senasaPuntos(producto.nombre),
    principio: senasaPuntos(producto.principioActivo),
    registro: escapeHtmlClient(senasaRegistroValor(producto)),
    metodologia: senasaPuntos(seccion.metodologia || seccion.metodo),
    frecuencia1: senasaPuntos(seccion.frecuenciaGrupo1 || seccion.frecuenciaVerificacion || seccion.frecuenciaHoras || seccion.frecuencia),
    sectores1: senasaPuntos(seccion.sectoresGrupo1),
    frecuencia2: senasaPuntos(seccion.frecuenciaGrupo2),
    sectores2: senasaPuntos(seccion.sectoresGrupo2)
  };
}
function senasaAvisoOriginalPreviewHtml(d = {}) {
  const r = senasaProductoTexto(d.roedores || {});
  const ie = senasaProductoTexto(d.insectosExternos || {});
  const ii = senasaProductoTexto(d.insectosInternos || {});
  const ov = senasaProductoTexto(d.otrasPlagas?.voladoras || d.otrasPlagas || {});
  const oc = senasaProductoTexto(d.otrasPlagas?.caminadoras || {});
  return `<h4>ROEDORES</h4><div class="senasa-box"><p>En el periodo comprendido del mes ${senasaPuntos(d.periodoDesde)} , se empleará como cebo rodenticida el producto: ${r.nombre}; cuyo principio activo es: ${r.principio}, aprobado por SENASA por la Resolución Nº ${r.registro}.</p><p>Las actividades de verificación y reposición se efectuarán con una frecuencia de ${r.frecuencia1}, en los sectores identificados en el plano de referencia con las letras ${r.sectores1} en tanto que en los sectores identificados con las letras ${r.sectores2} la frecuencia de verificación y reposición será de ${r.frecuencia2}.</p></div><h4>INSECTOS</h4><div class="senasa-box"><p><strong><u>SECTORES EXTERNOS:</u></strong> Periodo comprendido entre el ${senasaPeriodoTexto(d.insectosExternos || {}, d)}. Se empleará como <strong>insecticida para sectores externos</strong> el producto ${ie.nombre}. Cuyo principio activo es: ${ie.principio}. Y se encuentra aprobado por SENASA por Resolución Nº ${ie.registro}.</p><p>Las actividades se efectuarán con una frecuencia de ${ie.frecuencia1} horas, en los sectores identificados en el plano de referencia con las letras ${ie.sectores1} en tanto que los sectores identificados con las letras ${ie.sectores2} la frecuencia será de ${ie.frecuencia2} horas.</p><p><strong><u>SECTORES INTERNOS:</u></strong> Período comprendido entre el ${senasaPeriodoTexto(d.insectosInternos || {}, d)}. En <strong>sectores internos</strong> se utilizará en la sección identificada en el plano de referencia, usando el producto insecticida para uso interior ${ii.nombre} cuyo principio activo es: ${ii.principio}. Y se encuentra aprobado por SENASA por Resolución Nº ${ii.registro}.</p><p>La actividad se efectuará el día ${senasaPuntos(d.insectosInternos?.diaActividad)} a las ${senasaPuntos(d.insectosInternos?.horaActividad)} horas y se dejará actuar por un lapso de ${senasaPuntos(d.insectosInternos?.lapsoHoras)} horas.</p><p>Para proceder luego al lavado y desinfección de la sección</p></div><h4>OTRAS PLAGAS</h4><div class="senasa-box"><p><strong><u>SECTORES EXTERNOS ESPECIES VIVAS:</u></strong> Periodo comprendido entre el ${senasaPeriodoTexto(d.otrasPlagas?.voladoras || {}, d)}.</p><p>Se empleará para <strong>sectores externos</strong> la siguiente metodología/sistema de control ${ov.metodologia}. Que se encuentra desarrollado en el MIP.</p><p>Las actividades de control y verificación se efectuarán con una frecuencia de ${ov.frecuencia1}, en los sectores identificados en el plano de referencia con las letras ${ov.sectores1} en tanto que los sectores identificados con las letras ${ov.sectores2} la frecuencia será de ${ov.frecuencia2}.</p><p><strong><u>SECTORES EXTERNOS ESPECIES CAMINADORAS:</u></strong> Periodo comprendido entre el ${senasaPeriodoTexto(d.otrasPlagas?.caminadoras || {}, d)}.</p><p>Se empleará para <strong>sectores externos</strong> la siguiente metodología/sistema de control ${oc.metodologia}. Que se encuentra desarrollado en el MIP.</p><p>Las actividades de control y verificación se efectuarán con una frecuencia de ${oc.frecuencia1}, en los sectores identificados en el plano de referencia con las letras ${oc.sectores1} en tanto que los sectores identificados con las letras ${oc.sectores2} la frecuencia será de ${oc.frecuencia2}.</p></div><h4>AREAS EXTERNAS Y ESPACIOS VERDES</h4><div class="senasa-box"><p>Periodo comprendido entre el ${senasaPeriodoTexto(d.areasExternas || {}, d)}.</p><p><strong>Mantenimiento general de las áreas externas</strong> – (Acumulo de Basura, Chatarra, Malezas, etc.)</p><p>Las actividades de control y verificación se efectuarán con una frecuencia de ${senasaPuntos(d.areasExternas?.frecuenciaGrupo1 || d.areasExternas?.frecuencia)}, en los sectores identificados en el plano de referencia con las ${senasaPuntos(d.areasExternas?.sectoresGrupo1 || d.areasExternas?.sectoresMantenidos)}.</p><p><strong>Integridad del cerco perimetral y murete</strong> Las actividades de control y verificación se efectuarán con una frecuencia de ${senasaPuntos(d.areasExternas?.frecuenciaGrupo2)}, en los sectores identificados en el plano de referencia con las letras ${senasaPuntos(d.areasExternas?.sectoresGrupo2)}.</p></div><h4>HERMETICIDAD</h4><div class="senasa-box"><p>Periodo comprendido entre el ${senasaPeriodoTexto(d.hermeticidad || {}, d)}.</p><p>Las actividades de control y verificación del correcto funcionamiento de cortinas de aire, cierre automático de puertas, extractores de aire, cierres sinfónicos, mosquiteros, burletes y fuelles, electrocutores, etc.</p><p>Se efectuarán con una frecuencia de ${senasaPuntos(d.hermeticidad?.frecuenciaGrupo1 || d.hermeticidad?.frecuencia)}, en los sectores identificados en el plano de referencia con las ${senasaPuntos(d.hermeticidad?.sectoresGrupo1 || d.hermeticidad?.sectoresEvaluados)}.</p></div>`;
}
...
function renderSenasaPreview() {
  ...
  preview.innerHTML = `<div class="senasa-doc-title"><h2>${titulo}</h2><h3>${subtitulo}</h3><b>CIRCULAR N.º ${escapeHtmlClient(d.numeroCircular || '........')}</b></div>
...
function senasaProductoPayloadDesdePrompt() {
  setMsg('Los datos regulatorios SENASA / ANMAT se precargan automáticamente desde el producto oficial; no se solicitan al usuario.', 'info');
  return null;
}
